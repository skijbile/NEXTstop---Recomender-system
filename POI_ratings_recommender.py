...
def generate_recommendations(selected_user, selected_city, Events, suggested_events, restaurants, suggested_restaurant, POI, suggested_POI):
    recommendations = {
    ...
    if len(Events) > 0:
        ...
        recommendations['events'] = df1.head(10)
    else:
        df1 = pd.DataFrame(suggested_events)
        df1.reset_index(drop=True, inplace=True)
        df1.index = df1.index + 1
        recommendations['events'] = df1.head()

    if len(restaurants) > 0:
        df2 = pd.DataFrame(restaurants)
        df2.reset_index(drop=True, inplace=True)
        df2.index = df2.index + 1
        recommendations['restaurants'] = df2.head(10)
    else:
        df2 = pd.DataFrame(suggested_restaurant)
        df2.reset_index(drop=True, inplace=True)
        df2.index = df2.index + 1
        recommendations['restaurants'] = df2.head()
    ...
    if len(POI) > 0:
        ...
        if len(suggested_POI) > 0:
            ...
            df3.reset_index(drop=True, inplace=True)
            df3.index = df3.index + 1
            recommendations['suggested_POI'] = df3.head()
