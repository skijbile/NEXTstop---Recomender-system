...
from POI_ratings_recommender import main_recommender,process_output,load_users,pre_process,generate_recommendations # this .py file has the logic for the recommendations
...
# Check if recommendations are available in the cache
    if 'recommendation' not in st.session_state:
        st.session_state.recommendation = {}

    if selected_user in st.session_state.recommendation and selected_city in st.session_state.recommendation[selected_user]:
        recommendations = st.session_state.recommendation[selected_user][selected_city]
    else:
        recommendations = print_recommendations(selected_user, selected_city, Events, suggested_events, restaurants, suggested_restaurant, POI, suggested_POI)
        if selected_user not in st.session_state.recommendation:
            st.session_state.recommendation[selected_user] = {}
        st.session_state.recommendation[selected_user][selected_city] = recommendations
    placeholder.text("Printing Output!")
# Display recommendations
    st.write("Top Recommendations for:", recommendations['selected_user'])
    st.write("City:", recommendations['selected_city'])

    if 'events' in recommendations:
        st.write("Events in the area:")
        st.write(recommendations['events'])
    else:
        st.write("Other options:")
        st.write(recommendations['suggested_events'])

    if 'restaurants' in recommendations:
        st.write("Restaurants to visit:")
        st.write(recommendations['restaurants'])
    else:
        st.write("People have also liked:")
        st.write(recommendations['suggested_restaurant'])

    if 'POI' in recommendations:
        st.write("Places to visit:")
        st.write(recommendations['POI'])

    else:
        st.write("People have also visited:")
        st.write(recommendations['suggested_POI'])
